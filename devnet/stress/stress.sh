...
stress_precompile() {
    ...
    contender spam --builder-url "$BUILDER" --txs-per-second ${TPS:-50} --duration ${DURATION:-10} --seed $(echo "0x$(echo $(openssl rand -hex 32))") -p "$PRIVATE_KEY" './stress/scenarios/precompileStress.toml' -r "$BUILDER" --optimism --min-balance 0.7
}
