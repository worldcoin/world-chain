...
proof-deploy-nitro env="alphanet":
    ...
    export NITRO_DEPLOYMENT_OUT="${NITRO_DEPLOYMENT_OUT:-$(pwd)/pkg/contracts/deployments/{{env}}-nitro.json}"
    BROADCAST_FLAG=""
...
proof-deploy-system env="alphanet":
    ...
    export PROOF_SYSTEM_DEPLOYMENT_OUT="${PROOF_SYSTEM_DEPLOYMENT_OUT:-$(pwd)/pkg/contracts/deployments/{{env}}-proof-system.json}"
    BROADCAST_FLAG=""
    if [ "{{dry_run}}" = "false" ]; then
        BROADCAST_FLAG="--broadcast"
    fi
