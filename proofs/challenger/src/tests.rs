use crate::{
    ...
    ChallengerConfig, ChallengerError, ClaimOutcome, GameMetadata, OwnedGames, ResolutionManager,
    ResolutionManagerClient, ResolutionManagerConfig, ResolveSubmission,
...
impl ChallengerClient for MockClient {
    ...
    /// The mock reports a zero challenge period and surfaces each game's challenge deadline as
    /// its creation timestamp, so the cursor binary search probes the same values it always has.
