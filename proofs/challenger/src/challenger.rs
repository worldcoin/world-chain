    ...
    C: ConsensusProvider,
{
    async fn first_unexpired_game_index(
        ...
        // The stock factory interleaves every game type in one index sequence, so the probe
        // reads the factory's creation timestamp — defined at every index — instead of a World
        // Chain game's `challengeDeadline`. Both are monotonically non-decreasing in the index
        // and, for World Chain games, differ by exactly the constant challenge period.
        let challenge_period = self.execution_provider.challenge_period().await?;
        let mut low = 0;
        let mut high = game_count;

        while low < high {
            let middle = low + (high - low) / 2;
            let created_at = self.execution_provider.game_created_at(middle).await?;
            let deadline = created_at.saturating_add(challenge_period);
            if deadline <= now {
                low = middle + 1;
            } else {
                high = middle;
