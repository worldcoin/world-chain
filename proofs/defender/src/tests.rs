...
impl DefenderClient for MockClient {
    ...
    /// The mock reports a zero proof period and surfaces each game's proof deadline as its
    /// creation timestamp, so the cursor binary search probes the same values it always has.
    async fn game_created_at(&self, index: u64) -> Result<u64, DefenderError> {
        self.games
            .get(index as usize)
            .map(|game| game.proof_deadline)
            .ok_or_else(|| DefenderError::Contract(format!("unknown game index {index}")))
    }
