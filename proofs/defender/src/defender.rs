    ...
    P: ProofRequester + Sync,
{
    async fn first_unexpired_game_index(
        ...
        // Chain game's `proofDeadline`. The game sets `proofDeadline` at creation, so for World
        // Chain games the two differ by exactly the constant proof period.
        let proof_period = self.execution_provider.proof_period().await?;
        let mut low = 0;
        ...
        while low < high {
            ...
            let deadline = created_at.saturating_add(proof_period);
            if deadline <= now {
                low = middle + 1;
            } else {
                high = middle;
