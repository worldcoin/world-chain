...
/// The top-level prove args, forwarded from the xtask main.
pub type Args = ProveArgs;

pub async fn run(args: Args) -> eyre::Result<()> {
    let raw_tx_bytes = args.tx;
    let tx: TxEnvelope = TxEnvelope::decode(&mut raw_tx_bytes.as_ref())?;

    let tx_hash = tx.tx_hash();
    let signal_hash = hash_to_field(tx_hash.as_ref());

    let identity = args.identity_source.load();

    let inclusion_proof_proof_src = args.inclusion_proof_source.clone().into_variant();
    let inclusion_proof = match inclusion_proof_proof_src {
        ...
        InclusionProofSourceVariant::Url(url) => fetch_inclusion_proof(&url, &identity).await?,
    };
    ...
    let external_nullifier =
        ExternalNullifier::with_date_marker(date_marker, args.pbh_nonce as u16);
