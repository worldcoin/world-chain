...
contract DeployProofSystem is Script {
    ...
    function run() external returns (Deployment memory deployment) {
        ...
        deployment = _deploy(privateKey, config);
    ...
    function _deploy(uint256 privateKey, Config memory config) internal returns (Deployment memory deployment) {
        ...
        deployment.factory = _deployFactory(
            config, deployment.validityVerifier, deployment.teeVerifier, deployment.councilVerifier, deployment.staking
        );
    }
    ...
    function _deployFactory(
        ...
        MockRootIdVerifier councilVerifier,
        MockStakingRegistry staking
    ) internal returns (WorldChainProofSystemFactory) {
