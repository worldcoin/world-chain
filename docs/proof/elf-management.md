...
The first build runs each guest build inside the digest-pinned SP1 v6.8.1 image (a few minutes).
...
- Docker (default reproducibility mode pulls the digest-pinned `ghcr.io/succinctlabs/sp1:v6.8.1`), or
- The SP1 toolchain on `PATH` (`curl -L https://sp1.succinct.xyz | bash && sp1up --version v6.8.1`)
