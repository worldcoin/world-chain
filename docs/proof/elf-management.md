...
1. `proofs/succinct/utils/host/build.rs` calls
...
3. `proofs/succinct/utils/host/src/env_prover.rs` calls
   ...
   the prover binary at link time.

Net effect: the ELFs are never on disk for the host crate to find — they're statically baked
into every binary that uses `EnvSuccinctProver`. There is no committed ELF blob, no manifest of
SHA-256s, no `--range-elf` CLI flag, and no `RANGE_ELF_PATH` env var. The on-chain governance
anchor is the SP1 vkey computed from the embedded bytes (`just proof-vkeys`), which is exactly
what we register on `OPSuccinctFaultDisputeGame`.
...
1. Edit the guest source or bump the SP1 toolchain `tag` in `proofs/succinct/utils/host/build.rs`.
...
| Path | Role |
|:---|:---|
| `proofs/succinct/utils/host/build.rs` | Invokes `sp1_build::build_program_with_args` for each guest crate |
| `proofs/succinct/utils/host/src/env_prover.rs` | `range_elf()` / `aggregation_elf()` via `include_elf!()` |
