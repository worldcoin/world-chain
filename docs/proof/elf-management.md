...
`sp1_build::build_program_with_args` uses Docker by default with the SP1 v6.4.0 linux/amd64
...
The first build runs each guest build inside the digest-pinned SP1 v6.4.0 image (a few minutes).
...
- Docker (default reproducibility mode pulls the digest-pinned `succinctlabs/sp1:v6.4.0`), or
- The SP1 toolchain on `PATH` (`curl -L https://sp1.succinct.xyz | bash && sp1up --version v6.4.0`)
