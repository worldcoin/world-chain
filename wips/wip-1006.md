...
| `PROPOSER_BOND` | TBD | Bond tokens locked from the creator's available vault balance during factory creation. Paid to the proposer after `DEFENDER_WINS`, split between the challenger and protocol after a challenged proof timeout, paid to the protocol after an unchallenged proof timeout, or refunded if the registry invalidates the game. |
| `CHALLENGER_BOND` | TBD | Bond tokens locked from the challenger's available vault balance at challenge time. Split between the proposer and accepted proof-lane recipients after a successful defense, returned to the challenger after proof timeout, or refunded if the registry invalidates the game. |
...
The proposer deposits the configured bond token into `ERC20StakingVault` and calls `DisputeGameFactory.create(gameType, rootClaim, extraData)`, where:
...
1. The proposer deposits bond tokens into its available vault balance, then calls the stock `DisputeGameFactory.create` with zero ETH. The factory creates a clone, captures its caller as `gameCreator` and the L1 head, and calls `initialize`.
...
Resolution assigns normal-mode credits without releasing bond tokens. After the stock dispute-game finality delay, anyone MAY call `closeGame()` to fix the bond distribution mode:
...
| `IERC20StakingVault` | Accepts participant deposits, locks WIP-1006 proposal and challenge bonds from available balances, settles finalized game payouts, and enforces delayed token withdrawal. | [`IERC20StakingVault`](../pkg/contracts/src/dispute/interfaces/IERC20StakingVault.sol) |
