...
- **A diversified disputed path.** When a root is challenged, no single prover, TEE vendor, or council action can finalize it. At least two independent lanes must agree.
...
| `parentRef` | Proposer | Address of the parent — the `AnchorStateRegistry` for the first proposal, otherwise the parent proposal. Same convention as the OP Stack [Fault Dispute Game `extraData` parent reference][op-fdg] and Base Azul's [`AggregateVerifier.extraData()`][base-azul-extradata]. The parent's `rootClaim` and `l2BlockNumber` are read from this reference rather than re-supplied. |
| `intermediateRootsHash` | Proposer | Commitment to ordered intermediate output roots, if the implementation uses them. Compatible with Base Azul's intermediate-root commitment. |
| `l1OriginHash` | Factory | L1 origin hash captured at proposal creation. The proposer does not pass it as calldata; it is read via `blockhash()` or [EIP-2935][eip-2935] history and pinned by the factory, mirroring Base's [`l1Head()` snapshot at clone creation][base-azul-l1head]. |
...
All proofs and attestations MUST bind to `rootId`. A proof, game result, or attestation that binds to a different `domainHash`, parent, root, block range, or L1 origin MUST NOT be accepted for `rootId`.
...
Any staked participant MAY challenge a proposed root before `challengeDeadline`.
...
Anchor updates SHOULD be permissionless and self-validating once the root is finalized.

### Contract Interfaces

This WIP specifies interfaces only. Implementation details — storage layouts, clone-with-immutable-args payloads, journal encodings, verifier nullification, guardian controls, and bond escrow mechanics — follow Base Azul's [proof contracts specification][base-contracts] except where this WIP explicitly diverges.

| Interface | Role | Base reference |
| --- | --- | --- |
| `IDisputeGameFactory` | Creates per-proposal game clones; pins `l1OriginHash` at creation. | [`DisputeGameFactory`][base-factory] |
...
- `parentRef` follows the [Fault Dispute Game `extraData` parent reference convention](https://specs.optimism.io/fault-proof/stage-one/fault-dispute-game.html) — the `AnchorStateRegistry` for the first proposal, otherwise the parent proposal address.
