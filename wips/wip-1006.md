...
[base-azul]: https://docs.base.org/base-chain/specs/upgrades/azul/proofs
...
| --- | --- | --- |
| `IDisputeGameFactory` | Creates per-proposal game clones; pins `l1OriginHash` at creation. | OP Stack [`DisputeGameFactory`][op-dgi] |
| `IProofSystemGame` | Per-proposal game: holds state, accepts lane submissions, finalizes, advances anchor. World Chain analogue of Base's `AggregateVerifier`. | Base Azul [`AggregateVerifier`][base-azul] |
| `IAnchorStateRegistry` | Tracks the current anchor, finalized roots, blacklist, retirement, and pause. | OP Stack [`AnchorStateRegistry`][op-anchor] |
| `IValidityProofVerifier` | Verifies the `VALIDITY_PROOF` lane against `rootId`. | Base Azul [`ZKVerifier`][base-azul] |
...
| `ITEEVerifier` | Verifies the `TEE_ATTESTATION` lane against `rootId`. | Base Azul [`TEEVerifier`][base-azul] |
| `ITEEProverRegistry` | Maintains accepted TEE signer identities, image hashes, and proposer allowlisting. | Base Azul [`TEEProverRegistry`][base-azul] |
...
[op-anchor]: https://specs.optimism.io/fault-proof/stage-one/anchor-state-registry.html
