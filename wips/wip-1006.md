...
| `DISPUTE_GAME_FINALITY_DELAY` | `3.5 days` | Post-resolution airgap before a finalized game may advance the anchor or authorize Portal withdrawal finalization. This matches the OP Stack and Base default and gives the configured guardian time to pause and route an incident to governance. |
...
- MUST record `proofDeadline = block.timestamp + PROOF_PERIOD` on the root the first time it transitions to `CHALLENGED`. Subsequent challenges MUST NOT extend `proofDeadline`; implementations need not store a separate `challengedAt` value.
