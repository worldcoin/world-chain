...
| `PROOF_PERIOD` | `7 days` | Duration after a root is challenged during which the proposer MAY defend the root by accumulating lane submissions toward `PROOF_THRESHOLD`. On the first transition to `CHALLENGED`, the game sets `proofDeadline = block.timestamp + PROOF_PERIOD`. A challenged root that the proposer has not defended to `PROOF_THRESHOLD` by `proofDeadline` MUST be invalidated. |
...
- MUST record `proofDeadline = block.timestamp + PROOF_PERIOD`; implementations need not store a separate `challengedAt` value.
