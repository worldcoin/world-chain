...
$$w = \bigl(\mathit{id\_secret},\; \mathit{auth\_key},\; \mathit{path},\; \beta,\; R_{\mathrm{oprf}},\; \mathit{cred},\; \sigma_{\mathit{cred}}\bigr)$$

satisfying:

**(G1) Merkle membership.** $\mathit{auth\_key}$ is a leaf in the `WorldIDRegistry` tree at the declared $\mathit{root}$ and $\mathit{depth}$:

$$\mathrm{MerkleVerify}\!\bigl(\mathit{root},\; \mathit{leaf\_index},\; \mathrm{Poseidon}(\mathit{auth\_key}),\; \mathit{path}\bigr) = \mathsf{true}$$

**(G2) Credential validity.** $\mathit{cred}$ bears a valid EdDSA-Poseidon2 signature $\sigma_{\mathit{cred}}$ over [BabyJubJub](https://eips.ethereum.org/EIPS/eip-2494) from the issuer identified by $\mathit{issuerSchemaId}$, and $\mathit{cred.expiresAt} \geq \mathit{expiresAtMin}$.

**(G3) OPRF correctness.** The OPRF query $q$ was honestly derived from the user's leaf, the response $R_{\mathrm{oprf}}$ was evaluated under the registered OPRF key for $\mathit{rpId}$, and a DLog equality proof binds the blinded and unblinded responses.

**(G4) Deterministic nullifier derivation.**

$$q = \mathrm{Poseidon2}\!\bigl(\texttt{"World ID Query"},\; \mathit{leaf\_index},\; \mathit{rp\_id},\; \mathit{action}\bigr)$$

$$\nu = \mathrm{Poseidon2}\!\bigl(\texttt{"World ID Proof"},\; q,\; R_{\mathrm{oprf}}.x,\; R_{\mathrm{oprf}}.y\bigr)$$

The nullifier $\nu$ is deterministic per $(\mathit{leaf\_index},\; \mathit{rp\_id},\; \mathit{action})$. The same identity proving under the same $(\mathit{rp\_id},\; \mathit{action})$ always yields the same $\nu$, regardless of blinding factor $\beta$ or proof randomness.

**(G5) Signal binding.** $\mathit{signalHash}$ is "dummy-squared" inside the circuit (constrained to $\mathit{signalHash} \cdot \mathit{signalHash} = \mathit{signalHash}^2$), preventing the verifier from accepting a proof generated with a different signal.

**Soundness.** Under the knowledge soundness of Groth16 over $\mathbb{G}_{\mathrm{BN254}}$, an accepting proof guarantees the prover possesses a valid credential for a leaf in the committed tree, the OPRF evaluation was performed correctly under the registered key, and $\nu$ is deterministically bound to $(\mathit{leaf\_index},\; \mathit{rp\_id},\; \mathit{action})$. Distinct $\mathit{action}$ values yield distinct nullifier domains — a proof accepted under one action cannot be replayed under another.
...
Yields the **account nullifier** $\nu_a$, unique per $(\mathit{identity},\; g)$. The $\mathit{signalHash}$ commits to the authorized key set $K = \{k_0, \ldots, k_{n-1}\}$.
...
$$\mathrm{addr}(\nu_a) = \mathtt{bytes20}\!\bigl(\mathrm{keccak256}(\mathtt{abi.encodePacked}(\nu_a))\bigr)$$

Since $\nu_a$ is bound to $(\mathit{leaf\_index},\; \mathit{rpId},\; \mathit{action}_a)$ and $\mathit{action}_a$ encodes $g$, each $(\mathit{identity},\; g)$ pair maps to exactly one address.

---

#### 3.3.5 Account Recycling — Inductive Proof
...
**Definition 1** (Protocol state). Let $S_g$ denote the protocol state after generation $g$ has been created for a fixed identity with creation nullifier $\nu_c$. Define the state tuple:

$$S_g \;=\; \bigl(\,\mathtt{generations}[\nu_c],\;\; \mathtt{spentAccountNullifiers},\;\; \mathtt{destroyed}\,\bigr)$$

---

**Theorem 1** (Safety of account recycling). *For any identity with creation nullifier $\nu_c$, the `createAccount` protocol satisfies **Lemmas 1.1–1.4** at every reachable state $S_g$.*

---

**Proof.** By induction on the generation counter $g$.

**Base case** ($S_0$).

$\mathtt{generations}[\nu_c] = 0$. No account has been created. The spent-set and destroyed-set are empty with respect to this identity. All invariants hold vacuously. $\;\square_0$

**Inductive hypothesis.** Assume **Lemmas 1.1–1.4** hold at state $S_g$ for some $g \geq 0$.

**Inductive step** ($S_g \to S_{g+1}$). A `createAccount` call executes with proofs $\pi_c, \pi_a$ at generation $g$. We show each lemma is preserved.

> **Lemma 1.1** (Monotonicity).
>
> $$\forall\, \nu_c :\; \mathtt{generations}[\nu_c] \in \mathbb{N} \;\wedge\; \Delta\,\mathtt{generations}[\nu_c] = +1$$
>
> *Proof.* Step ⑦ of `createAccount` executes $\mathtt{generations}[\nu_c] \leftarrow g + 1$. The counter is only written in this single location and only incremented. By the inductive hypothesis $\mathtt{generations}[\nu_c] = g \in \mathbb{N}$, so after the step $\mathtt{generations}[\nu_c] = g + 1 \in \mathbb{N}$. $\;\square$

> **Lemma 1.2** (Nullifier uniqueness).
>
> $$\forall\, g :\; \mathtt{spentAccountNullifiers}[\nu_a(g)] = \mathsf{true} \;\implies\; \nexists\;\pi' \;\text{s.t.}\; \mathrm{Verify}(\pi')\;\text{outputs}\;\nu_a(g)$$
>
> *Proof.* Step ④ requires $\mathtt{spentAccountNullifiers}[\nu_a] = \mathsf{false}$ and then sets it to $\mathsf{true}$. Suppose for contradiction that a future proof $\pi'$ produces the same $\nu_a(g)$. By **(G4)**, $\nu_a$ is deterministic per $(\mathit{leaf\_index},\; \mathit{rpId},\; \mathit{action})$. Since $\mathit{action}_a(g)$ is fixed for generation $g$, any valid $\pi'$ yielding $\nu_a(g)$ must use the same action — but the spent-set guard rejects it. $\;\square$

> **Lemma 1.3** (Address freshness).
>
> $$g \neq g' \;\implies\; \mathrm{addr}(g) \neq \mathrm{addr}(g')$$
>
> *Proof.* By construction:
>
> $$\mathit{action}_a(g) = \mathrm{keccak256}\!\bigl(\mathtt{abi.encodePacked}(\mathit{rpId},\; 0,\; g)\bigr)$$
>
> Since $\mathrm{keccak256}$ is collision-resistant and $g$ varies, $g \neq g'$ implies $\mathit{action}_a(g) \neq \mathit{action}_a(g')$. Distinct actions yield distinct OPRF queries $q(g) \neq q(g')$ by injectivity of $\mathrm{Poseidon2}$ over distinct inputs. By the pseudorandomness of the OPRF, $\nu_a(g) \neq \nu_a(g')$ with overwhelming probability. Finally, $\mathrm{addr}(\cdot)$ applies $\mathrm{keccak256}$, preserving distinctness by collision resistance. $\;\square$

> **Lemma 1.4** (Destruction finality).
>
> $$\mathtt{destroyed}[\mathit{addr}] = \mathsf{true} \;\implies\; \mathit{addr}\;\text{is permanently excluded from initialization}$$
>
> *Proof.* Step ⑤ requires $\mathtt{destroyed}[\mathit{addr}] = \mathsf{false}$ before initialization. The `destroyed` mapping is write-once (set to $\mathsf{true}$ on destruction, never reset). By **Lemma 1.3**, generation $g + 1$ derives a fresh address $\mathrm{addr}(g+1) \neq \mathrm{addr}(g)$, so destruction at generation $g$ does not obstruct $S_{g+1}$. $\;\square$

Since **Lemmas 1.1–1.4** are preserved by the inductive step and hold at the base case, they hold for all reachable states $S_g$ where $g \in \mathbb{N}$. $\;\blacksquare$
...
$$\mathtt{account.storage}[\texttt{GENERATION\_SLOT}] \leftarrow g$$

$$\mathtt{account.storage}[\texttt{NUM\_KEYS\_SLOT}] \leftarrow |K|$$

$$\forall\, i \in [0,\, |K|) :\; \mathtt{account.storage}[\texttt{KEY\_SLOT}(i)] \leftarrow \mathrm{encode}(k_i)$$

$$\mathtt{account.storage}[\texttt{NULLIFIER\_SLOT}] \leftarrow \nu_a$$

$$\mathtt{account.code} \leftarrow \mathtt{0xef0100} \;\|\; \texttt{WORLD\_ID\_ACCOUNT\_DELEGATE}$$

#### 3.4.4 Signal Binding Invariant

The key set $K$ written to storage is **exactly** the set committed in $\pi_a$:

$$\mathit{signalHash}_a = \mathrm{keccak256}\!\bigl(\mathtt{abi.encode}(K)\bigr) \gg 8$$

By **(G5)**, the proof is bound to this $\mathit{signalHash}$. Any post-proof modification to $K$ would require a new accepting proof — which is impossible without a fresh $\mathit{action}_a(g')$ at a new generation. Key rotation therefore requires account recycling: $S_g \to S_{g+1}$.
