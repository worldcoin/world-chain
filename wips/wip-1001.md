...
- **Base case** $S_0$: $\mathtt{generations}[\nu_c] = 0$. No account exists.
- **Inductive step** $S_g \to S_{g+1}$: The account at generation $g$ is destroyed; a new `createAccount` call advances the counter.

**I1 — Monotonicity.**

$$\forall\, \nu_c :\; \mathtt{generations}[\nu_c] \in \mathbb{N} \;\wedge\; \Delta\,\mathtt{generations}[\nu_c] = +1 \;\text{per}\; \mathtt{createAccount}\;\text{call}$$

**I2 — Nullifier uniqueness.** Each $\nu_a(g)$ is spent exactly once:
...
$$g \neq g' \;\implies\; \mathit{action}_a(g) \neq \mathit{action}_a(g') \;\implies\; \nu_a(g) \neq \nu_a(g') \;\implies\; \mathrm{addr}(g) \neq \mathrm{addr}(g')$$

Holds because $\mathit{action}_a$ encodes $g$, producing a distinct OPRF query and therefore a distinct nullifier (by pseudorandomness of the OPRF and collision resistance of $\mathrm{Poseidon2}$).
