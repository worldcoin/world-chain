...
| `WORLD_CHAIN_ACCOUNT_BEACON` | `address` | `0x000000000000000000000000000000000000B0cC` | Predeploy address of the single global `WorldChainAccountUpgradeableBeacon`. Resolved by every `WorldChainAccountBeaconProxy` to locate the active account router implementation. |
| `WORLD_CHAIN_ACCOUNT_BEACON_PROXY_CODE_HASH` | `bytes32` | TBD | Runtime code hash of the `WorldChainAccountBeaconProxy` required at every World Chain account address. The proxy is constructed with `WORLD_CHAIN_ACCOUNT_BEACON` baked into its immutables, so this hash transitively binds every account to the canonical beacon. |
...
- the account address MUST have deployed runtime bytecode whose hash equals `WORLD_CHAIN_ACCOUNT_BEACON_PROXY_CODE_HASH`. That bytecode is the `WorldChainAccountBeaconProxy`, which transparently `DELEGATECALL`s the implementation returned by `WORLD_CHAIN_ACCOUNT_BEACON.implementation()`. All references in this document to "the account router" refer to this proxied router boundary at the account address;
...
After account creation, `setKeyRing` is the only execution path that mutates `sessionVerifiers`, `keyRingHash`, or validation-affecting account storage for active session verifiers. WIP-1001 defines no operation that mutates `admin`, the beacon proxy runtime code at the account address, or the `WORLD_CHAIN_ACCOUNT_BEACON` predeploy reference baked into that proxy. The account router implementation is upgraded out-of-band through `WORLD_CHAIN_ACCOUNT_BEACON`; see [Beacon-Proxied Account Router](#beacon-proxied-account-router).
...
account = address(uint160(uint256(keccak256(abi.encode(
    ...
    WORLD_CHAIN_ACCOUNT_BEACON_PROXY_CODE_HASH,
    adminHash,
    accountSalt
)))));
```

The address binds the account to the manager instance, chain, beacon proxy runtime code (and by extension the `WORLD_CHAIN_ACCOUNT_BEACON` it references), full admin verifier configuration, and salt. Because the binding is to the proxy's code hash rather than the implementation's, upgrading the account router implementation through `WORLD_CHAIN_ACCOUNT_BEACON` does NOT rotate any account address. WIP-1001 defines no operation that rotates or reinstalls `admin` for an existing account.
...
```solidity
IWorldChainAccount(account).isValidSignatureForAdmin(
    adminOperationHash,
    adminAuthorization
)
```
...
### Beacon-Proxied Account Router

The runtime bytecode at every account address is a `WorldChainAccountBeaconProxy`. The proxy is constructed with a single immutable reference to the `WORLD_CHAIN_ACCOUNT_BEACON` predeploy and forwards every call via `DELEGATECALL` to the implementation address returned by `WORLD_CHAIN_ACCOUNT_BEACON.implementation()`. The implementation is the `WorldChainAccount` contract; it carries the account router logic specified in [Account Router and Verifier Interfaces](#account-router-and-verifier-interfaces). Because the proxy stores all per-account state at the account address, upgrading the implementation does not migrate or relocate any account state.

The beacon and proxy compose as follows:

- `WORLD_CHAIN_ACCOUNT_BEACON` is a single global `WorldChainAccountUpgradeableBeacon` predeploy. Its `implementation()` returns the currently active `WorldChainAccount` implementation address. Implementation rotation is an out-of-band privileged operation performed by the beacon's owner and is NOT part of any per-account state transition defined in this WIP.
- `WORLD_CHAIN_ACCOUNT_BEACON_PROXY_CODE_HASH` is the canonical runtime code hash of `WorldChainAccountBeaconProxy` with the predeploy beacon address baked in. The manager installs exactly this bytecode at the account address during `create`; no other runtime bytecode satisfies the WIP-1001 account invariant.
- Every external entry point in [`IWorldChainAccount`](#account-router-and-verifier-interfaces) — `installAdmin`, `installKeyRing`, `isValidSignatureForAdmin`, `isValidSignatureForVerifier`, and `evaluateSessionPolicyForVerifier` — reaches the implementation through this proxy hop. The implementation's `MANAGER` immutable is checked on every state-mutating entry point so installation calls are restricted to `WORLD_CHAIN_ACCOUNT_MANAGER`.

Validation-affecting account state (the immutable admin verifier, the active `keyRingHash`, and per-verifier installation markers) lives in a deterministic [EIP-7201](https://eips.ethereum.org/EIPS/eip-7201) storage namespace at the account address. The implementation reads and writes this namespace through the proxy `DELEGATECALL`, so an implementation upgrade preserves the account's `admin`, `keyRingHash`, and session verifier set verbatim.

Implementation upgrades through `WORLD_CHAIN_ACCOUNT_BEACON` MUST NOT change the storage layout of the EIP-7201 namespace, MUST NOT change the `MANAGER` value baked into the implementation, and MUST NOT introduce new entry points that bypass the access control enforced on `installAdmin` and `installKeyRing`. Implementation upgrades that violate these invariants are out-of-spec; clients MAY treat detection of such an upgrade as a chain-level fault.
...
Admin authorization is valid only when `IWorldChainAccount(account).isValidSignatureForAdmin(adminOperationHash, adminAuthorization)` succeeds in a restricted validation frame and returns `EIP1271_MAGIC_VALUE`.

Installing the canonical `WorldChainAccountBeaconProxy` runtime bytecode (whose code hash equals `WORLD_CHAIN_ACCOUNT_BEACON_PROXY_CODE_HASH`) at the derived account address is a native manager state transition. It MUST NOT execute account-supplied init code and MUST NOT be represented as an EVM `CREATE` or `CREATE2` from the manager. The manager does NOT install the account router implementation per-account; the implementation is resolved at call time through `WORLD_CHAIN_ACCOUNT_BEACON`.
...
10. atomically install the canonical `WorldChainAccountBeaconProxy` runtime bytecode at `account` so its code hash equals `WORLD_CHAIN_ACCOUNT_BEACON_PROXY_CODE_HASH`;
11. call `IWorldChainAccount(account).installAdmin(admin)`;
12. call `IWorldChainAccount(account).installKeyRing(initialSessionVerifiers)`;
...
9. call `IWorldChainAccount(account).installKeyRing(sessionVerifiers)`;
...
For each EIP-1271 signature check, the protocol MUST perform a `STATICCALL` from `WORLD_CHAIN_ACCOUNT_MANAGER` to the `WorldChainAccountBeaconProxy` at `account` with value `0`, gas exactly `EIP1271_VALIDATION_GAS_LIMIT`, and calldata:

- `IWorldChainAccount.isValidSignatureForAdmin.selector || abi.encode(hash, signature)` for admin authorization checks; or
- `IWorldChainAccount.isValidSignatureForVerifier.selector || abi.encode(sessionVerifier, hash, signature)` for session signature checks.

The call succeeds only if it returns `EIP1271_MAGIC_VALUE` and triggers no tracer rule violation. The proxy transparently `DELEGATECALL`s the implementation resolved through `WORLD_CHAIN_ACCOUNT_BEACON`, which is responsible for `DELEGATECALL`ing the configured verifier implementation per the dispatch rules in "Account Router and Verifier Interfaces"; the manager never calls a verifier implementation directly.

For each session policy check, the protocol MUST perform a `STATICCALL` from `WORLD_CHAIN_ACCOUNT_MANAGER` to the `WorldChainAccountBeaconProxy` at `account` with value `0`, gas exactly `EXECUTION_TRACE_VALIDATION_GAS_LIMIT`, and calldata `IWorldChainAccount.evaluateSessionPolicyForVerifier.selector || abi.encode(sessionVerifier, context)`. The call succeeds only if it returns `true` and triggers no tracer rule violation. The proxy `DELEGATECALL`s the beacon-resolved implementation, which is responsible for `DELEGATECALL`ing the configured session verifier implementation.
...
6. call `IWorldChainAccount(account).isValidSignatureForVerifier(sessionVerifier, signingHash, signature)` in a restricted validation frame;
...
12. call `IWorldChainAccount(account).evaluateSessionPolicyForVerifier(sessionVerifier, context)` in a restricted validation frame;
...
- address derivation across multiple `(chainId, manager, adminSigner, accountSalt)` tuples, including verification that the derivation binds `WORLD_CHAIN_ACCOUNT_BEACON_PROXY_CODE_HASH` (not the implementation code hash) so that a beacon-driven implementation upgrade does NOT shift any account address;
- `WorldChainAccountBeaconProxy` deployment at the derived account address by `create`, including the runtime code-hash equality check against `WORLD_CHAIN_ACCOUNT_BEACON_PROXY_CODE_HASH` and rejection of any pre-existing bytecode at the address;
- beacon resolution: every state-mutating and validation entry point reaches the active `WorldChainAccount` implementation through `WORLD_CHAIN_ACCOUNT_BEACON.implementation()`, and rotating the implementation through the beacon owner preserves the account's EIP-7201 admin verifier, `keyRingHash`, and session verifier set verbatim;
...
- `DELEGATECALL` inside restricted validation frames is safe only when the delegatee bytecode satisfies the same tracer rules recursively. Every validation call traverses two `DELEGATECALL` hops at minimum: the beacon proxy to the beacon-resolved implementation, and the implementation to the configured verifier. Both hops MUST satisfy the tracer rules; the active `WorldChainAccount` implementation is therefore part of the validation-trust boundary.
- `WORLD_CHAIN_ACCOUNT_BEACON` is a privileged singleton. Whoever controls the beacon owner can rotate the implementation that every existing account delegates to. Beacon ownership MUST be controlled by a governance process commensurate with chain-level upgrades; an unauthorized implementation upgrade is equivalent to a chain-level compromise because it can rewrite admin and session validation semantics under preserved account storage. The proxy bytecode constant-binds `WORLD_CHAIN_ACCOUNT_BEACON`, so an account address cannot be re-pointed at a different beacon without changing the proxy code hash and therefore the account address itself.
- Implementation upgrades MUST preserve the EIP-7201 storage layout and the `MANAGER` immutable. A breaking layout change silently corrupts every existing account's admin and key ring state.
