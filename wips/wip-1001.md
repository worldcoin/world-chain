...
$$S_{g_t} = \bigl(\,\mathtt{generations}[\nu_c] = g_t,\;\; \mathtt{spentAccountNullifiers},\;\; \mathtt{destroyed}\,\bigr)$$
...
$$\forall\, \ell,\; \forall\, t :\; \mathbb{\bigl|\bigl\{\, \mathrm{addr}(g_\tau) \;\big|\; \tau \leq t \;\wedge\; \mathtt{destroyed}[\mathrm{addr}(g_\tau)] = \mathsf{false} \,\bigr\}\bigr|} \leq 1$$
...
*Lemma 1.5.* The creation proof $\pi_c$ uses constants $\mathit{action}_c = \mathrm{keccak256}(\texttt{"WORLD\_ID\_ACCOUNT\_CREATION"})$ and $\mathit{rpId} = 480$. By **(G4)**, $\nu_c = f(\ell,\; \mathit{rpId},\; \mathit{action}_c)$ is a unique function of $\ell$. Two distinct authenticators $\ell \neq \ell'$ produce $\nu_c \neq \nu_c'$ by pseudorandomness of the OPRF.
