...
description: A precompile-managed World ID Key Ring whose creation is gated by a World ID 4.0 Uniqueness Proof, whose subsequent mutations are gated by Session Proofs, and which is authenticated via a new EIP-2718 typed transaction (type `0x1D`).
...
Keyring state lives natively in the `WORLD_ID_KEYRING_PRECOMPILE`. There is no per-WorldID uniqueness constraint, no generation counter, and no account recycling — a *World ID Account* is a public identifier for a *World ID* managed set of session keys; it is not an identity anchor.
...
*Session keys* are abstract signatories authorized to sign `0x1D` transactions on behalf of the keyring; they carry no intrinsic privileges beyond signing. Per-key permissions (expiry, spend limits, call scopes) are out of scope for this WIP (see [Extensions](#extensions)).
...
- `signalHash = uint256(keccak256(abi.encode(update || WORLD_ID_KEYRING_NONCE)) >> 8`
...
- **`Create`** — installs `keys` as the full session key set at $\mathrm{addr}(\nu_k)$ and associates a fresh `sessionId` with the keyring. Does **NOT** store any `sessionNullifier`. MUST revert if the keyring already exists, if `keys.length == 0`, or if `keys.length > MAX_SESSION_KEYS`.
