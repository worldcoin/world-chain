...
The World Chain transaction validator is extended to enforce the virtual fee floor for in-scope transactions.
...
- the sender is verified in the address-book for the current epoch
- the transaction is a valid address-book registration transaction that verifies a specific address for the current epoch or next epoch

For address-book registration transactions:

- the subsidy is attributed to the address being verified, not necessarily `msg.sender`
- the registration transaction counts toward that address's subsidy budget for the current address-book epoch
- only canonical address-book registration calls are eligible

Exact address-book ABI and storage changes are out of scope here. A separate spec can define them in detail.

### 6. Subsidy Accounting

Each verified address has a subsidy budget denominated in wei for each address-book epoch.

- the subsidy budget uses the same epoch boundaries as the address-book
...
The address-book is extended to track subsidy usage per address per epoch.
...
At the end of each block, the builder sends a builder-owned transaction that updates the address-book with the subsidy deltas for that block.
...
World Chain clients should also expose account-aware subsidy information. A client can read the address-book state for an account and return at least:
...
The virtual fee parameters are committed one block ahead so all builders can validate the same fee floor for the next block. Using the address-book epoch for subsidy accounting keeps verification state and subsidy state aligned.
