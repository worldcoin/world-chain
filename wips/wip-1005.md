...
For World Chain, a **combination of Option B and Option E** is recommended.

**Option B** (Initiator Premium) should govern the *entry* cost: the challenger pays **1.5× the defender's required response bond** to open a dispute. This immediately prices out low-conviction spam while preserving economic viability for well-prepared honest challengers, who will recover their bond plus the defender's forfeited bond if they are correct.

**Option E** (Progressive Escalation Multiplier) should govern *subsequent rounds*: the escalating party must raise by **2×** while the responding party need only match at **1.5×**. This asymmetry ensures that a determined attacker faces exponentially growing capital requirements, while an honest party on the receiving end of escalation always has a cheaper matching cost — anchoring the Nash equilibrium firmly in favour of honest participation.

Option D (time-asymmetric windows) can be layered on top of this combination as a complementary protection, giving the defender a modest additional response window (e.g. 24 hours vs 12 hours for the challenger) without affecting bond sizing. Option C should be avoided: cheap challenger entry undermines the economic security of the entire game. Option A (pure symmetry) is acceptable as a fallback if governance prefers simplicity, but it provides weaker spam protection than Options B + E.

#### Parameter Recommendations for World Chain (under Option B + E)

| Parameter | Recommended Value | Rationale |
|---|---|---|
| Initial challenger bond | 0.15 ETH | 1.5× the defender entry bond; deters casual spam while remaining accessible |
| Initial defender bond | 0.1 ETH | Lower entry for reactive party; must still signal confidence |
| Escalation multiplier (escalating party) | 2.0× current bond | Aggressive escalation becomes exponentially costly for the attacker |
| Response multiplier (responding party) | 1.5× current bond | Cheaper to match than to initiate; preserves honest-party advantage |
| Maximum bond cap | 32 ETH | Matches a single Ethereum validator stake — a natural economic ceiling |
| Escalation response window (challenger) | 12 hours per round | Faster pressure on challenging party |
| Escalation response window (defender) | 24 hours per round | Option D complement; protects reactive defenders |
| Protocol fee | 3% of loser's bond | Funds prover incentive pool; low enough not to discourage honest participation |
