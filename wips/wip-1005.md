...
An attacker attempting to grief through N escalation rounds must stake:

```
attacker_cumulative_stake = B_initial × 1.5^N   (at minimum 1.5× increment per round)
```

At the bond cap of 32 ETH (with a 0.1 ETH initial bond and 1.5× increments), the attacker exhausts the cap in approximately **⌊log_{1.5}(320)⌋ = 13 rounds**. The attacker forfeits their entire cumulative stake on loss; the honest defender's net gain increases with each escalation round the attacker initiates.
...
### 9.6 Parameter Recommendations for World Chain

| Parameter | Recommended Value | Rationale |
|---|---|---|
| Initial challenger bond | 0.1 ETH | Low enough for legitimate challengers; high enough to deter casual spam |
| Initial defender bond | 0.1 ETH (match or exceed) | Symmetric entry; defender must signal equivalent confidence |
| Escalation increment | Minimum 1.5× current bond | Sub-exponential growth; keeps costs bounded at the cap |
| Maximum bond cap | 32 ETH | Matches a single Ethereum validator stake — a natural economic ceiling |
| Escalation response window | 24 hours per round | Gives both parties time to respond without unduly extending the challenge window |
| Protocol fee | 3% of loser's bond | Funds prover incentive pool; low enough not to discourage honest participation |
| Challenger asymmetry | Challenger pays 1.0× to open; 0.5× to respond to an escalation | Deters spam opens; cheap for honest challenger to respond if they were correct |
...
| Property | Value |
|---|---|
| Initial challenger bond | 0.1 ETH |
| Initial defender bond | 0.1 ETH (match or exceed) |
| Escalation increment | Minimum 1.5× current bond |
| Maximum bond cap | 32 ETH |
| Escalation response window | 24 hours per round |
...
| Griefing resistance | Attacker cumulative cost ∝ 1.5^N; capped at 32 ETH |
| Nash equilibrium | Honest parties always match; rational attackers never escalate |
