...
A Flashblock Access List (FAL) is an [EIP-7928][eip-7928]-style block access list scoped to a single flashblock. The builder records every account, balance, nonce, code, and storage change produced while executing a flashblock's transactions — each tagged with a block-local *block access index* — and publishes the resulting list, together with the Keccak256 hash of its RLP encoding, as an optional field on the flashblock pre-confirmation payload propagated over the Flashblocks P2P network.
...
- **Flashblock** — a single incremental pre-confirmation payload (`FlashblocksPayloadV1`) for an in-progress block, identified by `(payload_id, index)`. Flashblocks for one block carry contiguous indices starting at `0`; the `index = 0` flashblock additionally carries the immutable block base fields.
- **Diff** — the mutable portion of a flashblock (`ExecutionPayloadFlashblockDeltaV1`): the transactions added by this flashblock plus the resulting `state_root`, `receipts_root`, `logs_bloom`, `gas_used`, `withdrawals`, `withdrawals_root`, `block_hash`, and the OPTIONAL access list.
- **Block access index** — a block-local, monotonically increasing `u64` that totally orders state mutations within a block. Index `0` corresponds to pre-execution and system-level changes; each subsequent index corresponds to the execution of a single transaction in block order; the maximum index corresponds to post-execution system changes (for example fee-vault balance increments). Indices are assigned over the *entire block*, so a flashblock at a non-zero index continues the numbering of the flashblocks before it.

### Constants

The key words apply to the existing wire constant inherited from EIP-7928:

| Name | Value | Meaning |
| --- | --- | --- |
| `EMPTY_BLOCK_ACCESS_LIST_HASH` | `0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347` | Keccak256 of the RLP encoding of the empty EIP-7928 block access list, used in the reconstructed block header when the Amsterdam hardfork is active. |
...
**Why a hash rather than a Merkle commitment.** The access list is a pre-confirmation artifact, not part of the canonical block, and it is already covered by the builder's signature over the flashblock payload. A flat Keccak256 over the canonical RLP is the cheapest commitment that lets a consumer detect a builder that publishes an access list inconsistent with its own claimed state transition. Canonical form is what makes that hash reproducible.

**Why derive the state root from the diff but still re-execute.** Applying the diff to the parent bundle yields the post-state root directly and cheaply, which removes the state root from the execution critical path. Re-execution is retained purely as a soundness check: it independently reconstructs the access list and the receipts root, so a malformed or dishonest access list cannot pass validation even though the state root was taken from it.
...
Because the access list is never committed to in the canonical block header (the header records `EMPTY_BLOCK_ACCESS_LIST_HASH` under Amsterdam), it does not affect block hashes, state roots, or consensus, and it requires no coordinated hardfork to deploy or disable.

## Test Cases

Implementations SHOULD cover at least:

- a builder records balance, nonce, code, and storage post-values only for fields that actually changed, and records all storage reads;
- a newly-created account records its balance, nonce, and code at the creating transaction's index;
- canonicalization is deterministic: accounts sort by address, slots by key, per-field changes by `block_access_index`, consecutive identical writes collapse to the highest index, and all-zero/empty accounts are pruned;
- `access_list_hash` equals `keccak256(rlp(access_list))` over the canonical form, and re-deriving the list from independent execution reproduces the same hash;
- a temporal pre-state built from an access list returns, for a read at index `i`, the most recent write with index `< i`, and falls through to parent state for untouched accounts/slots/code;
- a flashblock diff round-trips through RLP with and without `access_list_data` present;
- validation accepts a correct flashblock and rejects one whose published `access_list_hash`, `receipts_root`, `state_root`, or `block_hash` does not match the re-derived value;
- a consumer reproduces the same `pending` block via the parallel path (access list present) and the legacy path (access list absent).

## Security Considerations

**The access list is advisory until verified.** A consumer MUST NOT trust a builder-supplied access list. Deriving the state root from the diff is safe only because the same flashblock is independently re-executed and its access list re-derived; the published `access_list_hash`, receipts root, state root, and block hash MUST all be checked before the flashblock is exposed as `pending`. A consumer that skips any of these checks can be induced to surface a state that the builder never validly produced.

**Canonicalization is part of the security boundary.** The hash check only detects tampering if both sides canonicalize identically. An implementation that orders, de-duplicates, or prunes differently will compute a different hash and either reject honest flashblocks or, worse, accept a non-canonical list that hashes to an attacker-chosen value. The canonical form rules MUST be implemented exactly.
