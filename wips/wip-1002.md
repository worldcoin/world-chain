...
Every World ID-verified user get's a subsidy budget denoted in Wei. The size of the budget depends on the subsidy-eligible World ID credentials the user posseses. For example, a user may claim a budget for it's `PoH Credential` from the Orb, one for it's `Phone Credential` from World App, and one for it's `Passport Credential`. Budgets refresh periodically, with a period length of 30 days.

Each period derives a unique World ID `action`, and users can claim their period budget and delegate the budget to a list of *authorized accounts* by submitting a `Uniqueness Proof`to a dedicated smart contract on World Chain. The identifier for the subsidy budget of a user, is the unique period nullifier. To allow the user to update the list of authorized accounts or claim additional credentials budgets after the claim, users also submit a  `Session ID`, s.t., later updates of the claim can be authenticated using World ID `Session Proofs`.

The logic to claim subsidies and authorize accounts, is facilitated as a Solidity pre-deploy. The logic to deduct the gas cost of subsidized transactions is implemented as EVM post-execution hook in [revm](https://github.com/bluealloy/revm), which directly updates the storage of the subsidies contract in-protocol.
...
- **An upgradeable Solidity proxy** (UUPS, OpenZeppelin-based) holds all canonical state and exposes every claim, authorization, governance, and view path: `claimSubsidy`, `updateRecord`, `setCredentialBudget`, and read methods.
...
- `auth`, `authorized`, `claimed`, `credentialBudget`, admin/governance slots, OpenZeppelin inherited state
...
`budget[action][nullifier] : uint128` is the remaining-Wei counter — the only slot the revm debit hook reads and writes. `getBudget(uint256 nullifier)` returns `budget[currentAction()][nullifier]`. `getBudget(address account)` sums `budget[currentAction()][nᵢ]` across every current-period claim the address is authorized under, mirroring the walk performed by the per-tx debit hook so off-chain callers observe the same effective budget. Credential-type budget configuration lives in a sibling governance map `credentialBudget : issuerSchemaId → amount`.

#### Auth Map

`auth[action][nullifier] : AuthRecord { sessionId, updateNonce }` is the Session-Proof gate for the per-claim state.

- `sessionId` is bound at `claimSubsidy` and retained for the period. A non-zero value is the existence marker: `claimSubsidy`'s "no prior record" check and `updateRecord`'s presence requirement both read it.
...
**Replay protection.** For `claimSubsidy` — `auth[currentAction()][nullifier].sessionId != 0` blocks re-opening in the same period. For `updateRecord` — the supplied `nonce` MUST equal the stored `auth[...][nullifier].updateNonce`; the contract bumps it on any successful state change, invalidating any earlier Session Proof against that record. The claimed-credentials map independently blocks re-claim of an already-claimed `issuerSchemaId` (the credential sub-op no-ops if already claimed). `signal_hash` binds the full operation context (`nullifier`, `nonce`, `issuerSchemaId`, `newSet`, `msgSender`), preventing cross-call replay.
...
The current design treats each period as a single sub-budget. Splitting the period into intra-period slots with proportional sub-budgets would smooth refresh congestion (see [Security Considerations](#security-considerations)) and bound the worst-case rate at which a single record can be drained. This is layered on top of the existing storage layout — slot accounting would be additive `AuthRecord` fields and an additional revm-side derivation — and is deferred as future scope.
