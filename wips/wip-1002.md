...
interface ISubsidyAccounting is IWIP101 {
    ...
    /// @notice Register the dense bitmap index for a subsidy-eligible credential type.
    ///         Governance only. Non-zero indexes are immutable once assigned.
    ///         MUST revert if `issuerSchemaId == 0`, if `index == 0`, if `issuerSchemaId`
    ///         already has a different non-zero index, or if `index` is already assigned
    ///         to a different credential.
    function registerCredentialBitIndex(uint64 issuerSchemaId, uint8 index) external;

    /// @notice Set the claimable budget amount (in Wei) for a credential type. Governance only.
    ///         MUST revert if `budgetWei != 0` and `credentialBitIndex[issuerSchemaId] == 0`.
...
Credential bit indexes are global, non-period-scoped state:

```solidity
mapping(uint64 issuerSchemaId => uint8 bitIndex) credentialBitIndex;
mapping(uint8 bitIndex => uint64 issuerSchemaId) credentialSchemaIdByBitIndex;
```
...
- `auth`, `authorized`, `credentialBudget`, `credentialBitIndex`, `credentialSchemaIdByBitIndex`, admin/governance slots
...
Subsidy-eligible credentials are assigned dense one-based bit indexes through `credentialBitIndex[issuerSchemaId]`. `credentialBitIndex == 0` means the credential is not subsidy-eligible. `issuerSchemaId == 0` MUST NOT be registered as subsidy-eligible. Non-zero indexes MUST be assigned with `registerCredentialBitIndex`, MUST be unique, and MUST NOT be reused for a different `issuerSchemaId`, even if a credential type is later deprecated. `credentialSchemaIdByBitIndex` is maintained to enforce uniqueness at registration time.
...
`hasClaimedCredential(nullifier, issuerSchemaId)` returns `false` if `credentialBitIndex[issuerSchemaId] == 0`; otherwise it returns whether the corresponding bit is set in the current-period `AuthRecord`.
...
**Credential bit-index governance.** Governance MUST register each non-zero `credentialBitIndex` once and MUST NOT reuse it for a different `issuerSchemaId`. Reusing an index could make historical claimed-credentials bitmaps ambiguous and could cause a record that already claimed one credential to appear to have claimed a different credential. The reverse `credentialSchemaIdByBitIndex` mapping exists so implementations can enforce this invariant on-chain.
