...
A **WorldID Gas Accounting** system enables per-credential gas subsidies for verified humans on World Chain. A [World ID](https://worldcoin.org/world-id) opens a per-period subsidy record using a World ID 4.0 [Uniqueness Proof](https://datatracker.ietf.org/doc/rfc9497/) whose `nullifier` keys the budget record and authorization state; the same per-period `nullifier` may carry multiple credential claims (via a bundled `ProofRequest`), and additional credentials acquired later in the period are added via Session Proofs. Budgets are governance-configurable per `issuerSchemaId`, authorized addresses may be added or removed throughout the period via Session Proofs, and subsidy records expire at period boundaries.
...
- **Authorization map:** maps authorized account addresses to `nullifier` values. One account can be mapped to multiple subsidy records.
...
- Initial subsidy claims (verify a per-period Uniqueness Proof, open the `nullifier` record with its `sessionId` and initial authorized addresses, and credit the first credential's budget)
...
interface IGasAccounting {
    /// @notice Initial per-period claim. Verifies a Uniqueness Proof for
    ///         `"period_proof" || periodNumber` and opens a subsidy record keyed by `nullifier`.
    ///         MAY be called multiple times for the same `nullifier` within a single
    ///         block using the per-credential proofs emitted by a bundled `ProofRequest`;
    ///         the first call establishes the record (storing `sessionId` and `addAddresses`),
    ///         subsequent calls only credit additional credentials.
    ///         MUST revert if `nullifier` is already recorded for a different period.
    ///         MUST revert if `issuerSchemaId` has already been claimed under `nullifier`.
    ///         MUST revert if the first call's `sessionId` does not match the stored `sessionId`
    ///         on subsequent calls for the same `nullifier`.
...
Maps authorized account addresses to `nullifier` values. One account can be mapped to multiple subsidy records. Authorized accounts can be added and removed throughout the entire period via `updateAddresses`. Authorized addresses MAY be EOAs, smart contract accounts, or [WIP-1001](./wip-1001.md) *World ID Key Rings*.

#### Gas Budget Map

Maps `nullifier` values to remaining budget in Gwei. Budget accumulates as the initial bundled proofs and later `claimAdditionalCredential` calls add credentials. Each `nullifier` record also stores the `sessionId` used to verify subsequent Session Proofs and the bitmap of claimed `issuerSchemaId` values. A helper lookup by authorized address MAY resolve the budget through the same deterministic nullifier-selection rule used during transaction execution.
...
Every Uniqueness Proof natively carries an `issuerSchemaId` (mandatory public input in the circuit). The nullifier is credential-independent: for a given World ID and period it does not depend on `issuerSchemaId`. To claim budget for multiple credentials against the same period nullifier, the authenticator builds a single `ProofRequest` containing one `RequestItem` per credential; this yields one per-credential proof for each `issuerSchemaId`, all sharing the same `nullifier` (see [World ID 4.0](https://github.com/worldcoin/world-id-protocol)). Each emitted proof is submitted on-chain via `claimSubsidy`.
...
Each method binds its non-ZK-committed call parameters into `signalHash` so the proof cannot be replayed with substituted parameters by a frontrunner. The `signalHash` MUST be constructed as follows (mirroring the [WIP-1001](./wip-1001.md) pattern — `>> 8` truncates to 248 bits so the value fits in the BN254 scalar field):
...
The component MUST recompute the expected `signalHash` from the call parameters and reject any proof whose `signalHash` public input does not match.

#### Per-Proof Behavior

For `claimSubsidy`:

1. Verify the Uniqueness Proof for `"period_proof" || periodNumber` with the recomputed `signalHash`.
2. If `nullifier` has no record in the current period: create the record, store `sessionId`, initialize the authorized-address set with `addAddresses`, and seed the claimed-credentials map as empty.
3. Otherwise require the stored `sessionId` to match the supplied `sessionId`.
4. Require `issuerSchemaId` to not already be in the claimed-credentials map for this `nullifier`; mark it as claimed and add its configured budget to `nullifier`'s remaining budget.

For `claimAdditionalCredential`:

1. Require `nullifier` to have a record in the current period; load its `sessionId`.
2. Verify a Session Proof against `sessionId` with the recomputed `signalHash`.
3. Require `issuerSchemaId` to not already be in the claimed-credentials map for this `nullifier`; mark it as claimed and add its configured budget.

For `updateAddresses`:

1. Require `nullifier` to have a record in the current period; load its `sessionId`.
2. Verify a Session Proof against `sessionId` with the recomputed `signalHash`.
3. Apply `addAddresses` and `removeAddresses` to the authorized-address set.
...
**Step 1: Initial claim — bundle PoH + Phone in a single `ProofRequest`.**
...
This yields two per-credential Uniqueness Proofs sharing one `nullifier`. The user submits both via `claimSubsidy`:

```
claimSubsidy(nullifier, 0x01, sessionId, proof_PoH,   signal_PoH,   [0xAlice, 0xBob]);
claimSubsidy(nullifier, 0x02, sessionId, proof_Phone, signal_Phone, []);
```

The component:

1. First call — `nullifier` has no record for period 7 → create the record, store `sessionId`, authorize `[0xAlice, 0xBob]`, mark `0x01` claimed, credit 50,000 Gwei.
2. Second call — `nullifier` record exists, `sessionId` matches; mark `0x02` claimed, credit 10,000 Gwei.

State after step 1: `nullifier` record has budget 60,000 Gwei, authorized = `[0xAlice, 0xBob]`, claimed = `{0x01, 0x02}`.

**Step 2: Mid-period — acquire NFC credential and claim its budget.**

Later in period 7 the user obtains an NFC credential. The authenticator generates a Session Proof against the stored `sessionId` with a fresh `sessionNullifier2`, for `issuerSchemaId = 0x03`, with a signal binding `"claimAdditionalCredential"`, `nullifier`, `msg.sender`, and a fresh `nonce`. The user calls:

```
claimAdditionalCredential(nullifier, 0x03, sessionNullifier2, proof, signal);
```
...
State after step 2: budget = 80,000 Gwei, authorized = `[0xAlice, 0xBob]`, claimed = `{0x01, 0x02, 0x03}`.

**Step 3: Session Proof — revoke an address.**

The user wants to remove `0xBob`. They generate another Session Proof against `sessionId` with a fresh `sessionNullifier3` and signal binding `"updateAddresses"`, `nullifier`, the address lists, `msg.sender`, and `nonce`:

```
updateAddresses(nullifier, 0x01, sessionNullifier3, proof, signal, [], [0xBob]);
```

The component verifies the Session Proof and removes `0xBob` from `nullifier`'s authorized set.

State after step 3: budget = 80,000 Gwei (minus any consumed), authorized = `[0xAlice]`, claimed = `{0x01, 0x02, 0x03}`.
...
**Bundled initial claim + Session Proof additions.** World ID 4.0 authenticators refuse to reissue a `(rpId, action)` nullifier across separate `ProofRequest`s within a period. Collecting all currently-held credentials in one bundled `ProofRequest` at initial claim, and admitting later-acquired credentials via Session Proofs against the stored `sessionId`, gives a spec-compliant path for both the initial multi-credential claim and mid-period additions without requiring a second Uniqueness Proof for the same period.
...
**Signal binding against frontrunning.** Each method binds its non-ZK-committed call parameters (`sessionId`, `nullifier`, address lists, `msg.sender`, `nonce`) into `signalHash`. World ID 4.0 dummy-squares the signal inside the circuit, so substituting any bound parameter invalidates the proof. This mirrors the [WIP-1001](./wip-1001.md) signal-binding pattern.
...
**Signal binding.** `signalHash` MUST commit to all non-ZK-committed call parameters (`sessionId`, `nullifier`, address lists, `msg.sender`, `nonce`) so a frontrunner cannot replay a witnessed proof with a substituted `msg.sender` or authorized-address list. Session Proofs may still be replayed in-flight by a griefer, but signal binding ensures the resulting state transition is identical to the one the original submitter intended.
