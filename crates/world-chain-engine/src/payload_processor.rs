...
use world_chain_builder::bal_validator::{
    FlashblocksBlockValidator, decode_transactions_with_indices,
};
...
impl WorldChainPayloadProcessor {
    ...
    pub fn new(
        ...
        let inner = Arc::new(RwLock::new(WorldChainPayloadProcessorInner {
    ...
    where
        Node: FullNodeTypes,
        Node::Provider:
            StateProviderFactory + HeaderProvider<Header = Header> + CanonStateSubscriptions,
...
/// Core flashblock processing loop extracted from [`WorldChainPayloadProcessor::launch`].
///
...
pub async fn run_flashblock_processor<T, S, Provider>(
    processor: Arc<WorldChainPayloadProcessor>,
    stream: S,
    provider: Provider,
...
pub fn process_flashblock<Provider>(
    provider: Provider,
    evm_config: &OpEvmConfig,
    processor: &WorldChainPayloadProcessor,
    chain_spec: Arc<OpChainSpec>,
        ...
        + 'static,
{
    ...
    let (base, is_new_epoch) = {
        let inner = processor.inner.read();
    ...
    if is_new_epoch {
        processor.inner.write().latest_payload = None;
    }
    ...
    let validate_span = MetricsSpan::new(
        tracing::trace_span!(
            target: "flashblocks::coordinator",
            "validate",
    ...
    {
        let mut inner = processor.inner.write();
    ...
    let payload_events = processor.inner.read().payload_events.clone();
    processor.broadcast_payload(Events::BuiltPayload(payload), payload_events)?;
