//! L1 provider builder.
//!
//! Copied from the proposer crate (`world-chain-exex::provider`). Produces a
//! [`DynProvider`] with fallback across endpoints, retry/rate-limit backoff,
//! default + fallback gas estimation, cached monotonic nonce management,
//! `chain_id` pre-fetch, and an [`EthereumWallet`] signer (the batcher EOA).
//!
//! The batcher EOA configured here MUST correspond to the on-chain
//! `SystemConfig.batcherHash`, or the derivation pipeline will reject the
//! submitted batches.
