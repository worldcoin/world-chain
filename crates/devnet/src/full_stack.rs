...
/// indexed `DisputeGameFactory` games of the World Chain game type, recomputes the
/// expected output root from the op-node rollup RPC, and challenges any game whose
...
fn build_components(
    ...
    if let Some(deployment) = proof_system {
        ...
        components.push(
            ...
            .with_endpoint("factory", deployment.proof_system_factory.clone())
            .with_note(
                "native in-process challenger that disputes invalid DisputeGameFactory games via MultiProofGame.challenge",
            )
            .with_note(
