...
///
/// Used as the default when [`BLOCK_NUMBER_ENV`] is not set. Change this
/// constant locally (or override at runtime via the env var) when switching the
/// captured mainnet block used by the bench.
///
/// IMPORTANT: the node database referenced by [`DATADIR_ENV`] (or the platform
/// default datadir) MUST be unwound to exactly `bench_block_number() - 1`
/// before running this bench. Otherwise state lookups go through
...
/// Optional override for the bench block number.
///
/// When set to a valid `u64`, the bench loads flashblocks for that block
/// instead of [`BENCH_BLOCK_NUMBER`]. When unset, the constant default is used.
const BLOCK_NUMBER_ENV: &str = "BLOCK_NUMBER";
...
/// Resolves the bench block number, preferring the [`BLOCK_NUMBER_ENV`]
/// override over the [`BENCH_BLOCK_NUMBER`] default.
fn bench_block_number() -> u64 {
    match std::env::var(BLOCK_NUMBER_ENV) {
        Ok(value) => value.parse::<u64>().unwrap_or_else(|err| {
            panic!("invalid {BLOCK_NUMBER_ENV} value {value:?}: {err}");
        }),
        Err(std::env::VarError::NotPresent) => BENCH_BLOCK_NUMBER,
        Err(err) => panic!("failed to read {BLOCK_NUMBER_ENV}: {err}"),
    }
}
...
/// flashblock sequence for the resolved bench block is processed by the same
/// coordinator because later flashblocks depend on the state produced by
/// earlier ones.
...
fn bench_process_flashblock_mainnet_block(c: &mut Criterion) {
    ...
    let block_number = bench_block_number();
    let stored = load_flashblocks_for_block(paths.flashblocks_db, block_number)
    ...
    println!(
        "flashblock_validation_mainnet: block {block_number} contains {} flashblocks",
    ...
    group.bench_function(BenchmarkId::new("block", block_number), |b| {
        b.iter_batched(
            || {
