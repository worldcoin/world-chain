...
pub enum OpPayloadBuilderError {
    ...
    /// Thrown when the upstream OP payload builder implementation is requested from the vendored
    /// payload-types crate.
    #[error("OP payload builder implementation is not vendored in World Chain")]
    PayloadBuilderUnavailable,
}
