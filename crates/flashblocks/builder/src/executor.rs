    ...
    Spec: OpHardforks + Clone,
{
    ...
    pub fn with_receipts(mut self, receipts: Vec<R::Receipt>) -> Self {
        self.inner.receipts.extend(receipts);
        self
    }
...
pub struct FlashblocksBlockBuilder<'a, N: NodePrimitives, Evm> {
    pub inner: BasicBlockBuilder<
        ...
        N,
    >,
    pub access_list: FlashblockBlockAccessList,
...
where
    ...
    >,
{
    /// Finishes the block builder and returns the outcome along with the access list.
    pub fn finish_with_access_list(
        self,
        state: impl StateProvider,
    ) -> Result<(BlockBuilderOutcome<N>, FlashblockBlockAccessList), BlockExecutionError> {
        let access_list = self.access_list.clone();
        let outcome = self.finish(state)?;
    ...
    pub fn update_access_list(&mut self, state: EvmState) {
        ...
        self.transaction_index = new_index;
    }
}
