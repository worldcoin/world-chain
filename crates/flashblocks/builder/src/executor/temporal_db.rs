            ...
            for storage_change in change.storage_changes {
                for slot in storage_change.changes {
                    ...
                    storage_entry.insert(
                        ...
                        // ANSWER: the +1 is because we're creating a `TemporalState` that
                        // will act as db. So we're essentially saying that tx `block_access_index + 1`
                        // will need to use as storage value the value that has been updated during the previous tx
