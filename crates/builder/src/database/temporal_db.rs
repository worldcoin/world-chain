...
impl TemporalDbFactory {
    ...
    pub fn new<DB: DatabaseRef>(db: &DB, list: &FlashblockAccessList) -> Self {
        let mut cache = TemporalState::default();

        for change in &list.changes {
            // First Grab the current account
            ...
            for balance_change in &change.balance_changes {
                let mut acc = acc_at(&cache, balance_change.block_access_index + 1);
                acc.balance = balance_change.post_balance;
            ...
            for nonce_change in &change.nonce_changes {
                let mut acc = acc_at(&cache, nonce_change.block_access_index + 1);
                acc.nonce = nonce_change.new_nonce;
                cache
                    .account_info
                    .insert(nonce_change.block_access_index + 1, change.address, acc);
            }
            ...
            for code_change in &change.code_changes {
                ...
                let mut acc = acc_at(&cache, code_change.block_access_index + 1);
                acc.code = Some(bytecode.clone());
                acc.code_hash = hash;

                cache
                    .contracts
                    .insert(code_change.block_access_index + 1, hash, bytecode);

                cache
                    .account_info
                    .insert(code_change.block_access_index + 1, change.address, acc);
            }
        }
    ...
    pub fn db<DB: DatabaseRef>(&self, db: DB, index: u64) -> TemporalDb<DB> {
        TemporalDb::new(db, self.cache.clone(), index)
    }
}
...
impl<DB: DatabaseRef> TemporalDb<DB> {
    pub fn new(db: DB, cache: Arc<TemporalState>, index: u64) -> Self {
...
mod tests {
    ...
    fn test_temporal_db_factory_with_multiple_changes() {
        ...
        let temporal_db = factory.db(&db, 4);
        dbg!(&temporal_db);
    ...
    fn test_temporal_db_basic_ref_fallback_to_db() {
        ...
        let temporal_db = factory.db(&db, 0);
    ...
    fn construct_db() {
        ...
        let fallback_db = InMemoryDB::default();

        let database = TemporalDbFactory::new(&fallback_db, &access_list.access_list.clone());

        // Test at block access index 3 - should see values < 3 (i.e., indices 0, 1, 2)
        let db = database.db(&fallback_db, 3);
        ...
        let db_0 = database.db(&fallback_db, 0);
        ...
        let db_1 = database.db(&fallback_db, 1);
        ...
        let db_10 = database.db(&fallback_db, 10);
        ...
        let db_11 = database.db(&fallback_db, 11);
        ...
        let db_5 = database.db(&fallback_db, 5);
        ...
        let db_12 = database.db(&fallback_db, 12);
